...
POSITIVE VARIABLES
    K(year_all)                Capital stock in period year
    KN(year_all)               New Capital vintage in period year
    ...
    C(year_all)                Consumption (Trillion $)
    I(year_all)                Investment (Trillion $)
;
...
EQUATIONS
    ...
    CAPITAL_CONSTRAINT(year_all)    Capital constraint

    NEW_CAPITAL(year_all)           New capital
    NEW_PRODUCTION(year_all)        New production

    TOTAL_CAPITAL(year_all)         Total capital stock across all vintages
    TOTAL_PRODUCTION(year_all)      Total production across all vintages

    NEW_ENERGY(sector, year_all)    New end-use services or commodities (production function)
...
UTILITY =E=
           (SUM(year $ (NOT macro_base_period(year) AND NOT last_period(year)),
                udf(year) * LOG(C(year)) * duration_period)
          + SUM(year $ last_period(year),
                udf(year) * LOG(C(year)) * (duration_period + 1/finite_time_corr(year))))
;
...
CAPITAL_CONSTRAINT(year)..
Y(year) =E=
C(year) + I(year) + EC(year)
;
...
NEW_CAPITAL(year) $ (NOT macro_base_period(year))..
KN(year) =E= duration_period * I(year)
;
...
*
***
NEW_PRODUCTION(year) $ (NOT macro_base_period(year))..
YN(year) =E=
( a * KN(year)**(rho * kpvs) * newlab(year)**(rho * (1 - kpvs))
+ b * NEWENE("ELEC", year)**(rho * elvs) * NEWENE("NELE", year)**(rho * (1 - elvs)) )**(1/rho)
;

$ONTEXT
NEW_PRODUCTION(year) $ (NOT macro_base_period(year))..
YN(year) =E=
( LAKL * KN(year)**(rho * kpvs) * newlab(year)**(RHO * (1 - kpvs))
+ SUM(sector, PRFCONST(sector) * NEWENE(sector, year)**rho) )**(1/rho)
;
$OFFTEXT
...
TOTAL_CAPITAL(year) $ (NOT macro_base_period(year))..
K(year) =E=
SUM(year2$( seq_period(year2,year) ), K(year2)) * (1 - depr)**duration_period + KN(year)
;
...
TOTAL_PRODUCTION(year) $ (NOT macro_base_period(year))..
Y(year) =E=
SUM(year2$( seq_period(year2,year) ), Y(year2)) * (1 - depr)**duration_period + YN(year)
;
...
NEW_ENERGY(sector, year) $ (NOT macro_base_period(year))..
PRODENE(sector, year) =E=
SUM(year2$( seq_period(year2,year) ), PRODENE(sector, year2)) * (1 - depr)**duration_period + NEWENE(sector, year)
;
...
ENERGY_SUPPLY(sector, year) $ (NOT macro_base_period(year))..
PHYSENE(sector, year) =G=
PRODENE(sector, year) * aeei_factor(sector, year)
;
...
TERMINAL_CONDITION(last_period)..
I(last_period) =G= K(last_period) * (grow(last_period) + depr)
;
